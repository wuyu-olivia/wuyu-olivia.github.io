...
MAX_LEN = 256
SEED = 1024
EVAL_SCHEDULE = [(0.50, 24), (0.49, 12), (0.48, 6), (0.47, 3), (-1., 3)]
NAME = sys.argv[1] #deberta, roberta, electra
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
DATA_PATH = sys.argv[2] 
VERSION = sys.argv[3]
HEAD = sys.argv[4] #att_la, att_l1, att_l2, att_l4, att_l2_1, att_l4_1, cnn_la, cnn_l1, cnn_l2, cnn_l4, cnn_l2_1, cnn_l4_1
          #dense_la, dense_l1, dense_l2, dense_l4, dense_l2_1, dense_l4_1, wp_l2, wp_l4, wp_l9
if NAME == 'deberta':
    ROBERTA_PATH = "microsoft/deberta-base"
    TOKENIZER_PATH = "microsoft/deberta-base"
elif NAME == 'roberta':
    ROBERTA_PATH = "roberta-base"
    TOKENIZER_PATH = "roberta-base"
else:
    ROBERTA_PATH = "google/electra-base-discriminator"
...
def train_folds(head='', model_name=None):
    ...
    for fold, (train_indices, val_indices) in enumerate(kfold.split(train_df)):    
        ...
        model_path = f"{NAME}_base_{VERSION}_{head}_f{fold + 1}.pth"
...
if HEAD == 'att_la':
    ...
    cv = train_folds(head=HEAD, model_name=AttentionModel_l2)
elif HEAD == 'att_l4':
    cv = train_folds(head=HEAD, model_name=AttentionModel_l4)
elif HEAD == 'att_l2_1':
    cv = train_folds(head=HEAD, model_name=AttentionModel_l2_1)
elif HEAD == 'att_l4_1':
    cv = train_folds(head=HEAD, model_name=AttentionModel_l4_1)
elif HEAD == 'cnn_la':
    cv = train_folds(head=HEAD, model_name=CNNModel_la)
elif HEAD == 'cnn_l1':
    cv = train_folds(head=HEAD, model_name=CNNModel_l1)
elif HEAD == 'cnn_l2':
    cv = train_folds(head=HEAD, model_name=CNNModel_l2)
elif HEAD == 'cnn_l4':
    cv = train_folds(head=HEAD, model_name=CNNModel_l4)
elif HEAD == 'cnn_l2_1':
    ...
    cv = train_folds(head=HEAD, model_name=DenseModel_l2)
elif HEAD == 'dense_l4':
    cv = train_folds(head=HEAD, model_name=DenseModel_l4)
    ...
    cv = train_folds(head=HEAD, model_name=WeightPoolModel_l2)
elif HEAD == 'wp_l4':
    cv = train_folds(head=HEAD, model_name=WeightPoolModel_l4)
elif HEAD == 'wp_l9':
    cv = train_folds(head=HEAD, model_name=WeightPoolModel_l9)
cv.append(np.mean(cv))
pd.Series(cv).to_csv(f'{NAME}_base_score_{VERSION}_{HEAD}.csv', index=False)